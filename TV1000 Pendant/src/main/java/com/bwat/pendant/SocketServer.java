...
public class SocketServer {
			...
			public void run() {
				while (true) {
					...
					try {
						...
						AGVUtils.logD("Client " + client + "connected");
					} catch (IOException e) {
						AGVUtils.logD(e.getMessage());
					}
					Thread.yield();
				}
			}
	...
	public String[] sendAll(String message, boolean waitForResponse) {
		AGVUtils.logD("Sending string \"" + message + "\" to all clients");
	...
	public class Connection {
				...
				public void run() {
					while (running) {
						...
						try {
							...
							AGVUtils.logD("Message received: " + response);
							waiting = false;
						} catch (IOException e) {
							AGVUtils.logD("Error reading from input: " + e.getMessage());
						}
						Thread.yield();
					}
				}
		...
		public void disconnect() {
			try {
				...
				socket.close();
				clients.remove(this);
				AGVUtils.logD(socket + " disconnected");
			} catch (IOException e) {
				AGVUtils.logD("Error disconnecting: " + e.getMessage());
			}
		}
		...
		public String send(String message, boolean waitForResponse) {
			...
			writer.flush();
			AGVUtils.logD("String \"" + message + "\" sent to " + socket);
			if (waitForResponse) {
				waiting = true;
				AGVUtils.logD("Waiting for response from: " + socket);
